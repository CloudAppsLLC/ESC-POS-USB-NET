...
namespace ESC_POS_USB_NET
{
    public class IPrinting
    {
        public bool PrintTestPage(string printer_name, string text)
        {
            try
            {
                ...
                recordDoc.PrintPage += new PrintPageEventHandler((sender2, e2) => PrintTestPageHandler(sender2, e2, text)); // function below
                recordDoc.PrintController = new StandardPrintController(); // hides status dialog popup
                                                                           //Main Printer
                if (!string.IsNullOrEmpty(printer_name))
                {
                    recordDoc.PrinterSettings.PrinterName = printer_name;
                    recordDoc.Print();
                    recordDoc.Dispose();
                }
            }
