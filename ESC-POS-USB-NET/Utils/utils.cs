using Newtonsoft.Json.Linq;
using System;
using System.Drawing;
using System.IO;
using System.Threading.Tasks;
using ESC_POS_USB_NET.Enums;



namespace ESC_POS_USB_NET.Utils
{

    class utils
    {

       
        public static string LazyWaitPOS = "";
          
        public static string GetTranslation(string key, string lan)
        {
            var result = key;

            try
            {
                    if (PrintingDictionary.Terms[key] != null && PrintingDictionary.Terms[key][lan] != null)
                    {
                         result = PrintingDictionary.Terms[key][lan].Value<string>();
                    }
              
                return result;
            }
            catch
            {
                return key;
            }

        }

        public static void PrintCanvasAsync(MemoryStream ms, Graphics canvas, ESC_POS_USB_NET.Printer.Printer printer, int width = -1)
        {
            
                
        }

       
        public static void PrintReceiptCanvasAsync(Order order, string type, MemoryStream ms, Graphics canvas, ESC_POS_USB_NET.Printer.Printer printer, int width = -1, bool? bill_show_logo = false, string logo = "", bool? bill_show_barcode = false, bool? receipt_show_barcode = false)
        {

         
                byte[] lwLogo = Convert.FromBase64String(LazyWaitPOS);
         //       await stream.ReadAsync(bytes, 0, bytes.Length);

                printer.AlignCenter();

                bool show_logo = true;
                if (type == "invoice") 
                {
                    show_logo = bill_show_logo ?? false;
                }

                if (show_logo)
                {
                    if ((!String.IsNullOrWhiteSpace(logo) || !String.IsNullOrEmpty(logo)) && logo.Contains("base64") && !logo.Contains("jpeg"))
                    {
                        logo = logo.Substring(22);
                        byte[] logoBytes = Convert.FromBase64String(logo);
                        printer.Image(logoBytes, 200, true);
                    }
                }
               
                //printer.Image(bytes, width, true);
                bool print_barcode = bill_show_barcode ?? false;

                if(type == "receipt")
                {
                    print_barcode = receipt_show_barcode ?? false;
                }

                if (print_barcode)
                {
                    printer.NewLine();
                    printer.PrintBarcode(ESC_POS_USB_NET.Enums.BarcodeType.CODE128, order.order_id, 50);
                }

                if (!string.IsNullOrEmpty(order.qr)) {
                    printer.NewLine();
                    //QRCodeGenerator qrGenerator = new QRCodeGenerator();
                    //QRCodeData qrCodeData = qrGenerator.CreateQrCode(order.qr, QRCodeGenerator.ECCLevel.Q);
                    //BitmapByteQRCode qrCode = new BitmapByteQRCode(qrCodeData);
                    //byte[] qrCodeAsBitmapByteArr = qrCode.GetGraphic(1);
                    //printer.NewLine();
                    //printer.Image(qrCodeAsBitmapByteArr, 200,true);
                    //printer.NewLine();
                }
                printer.Image(lwLogo, 170, true);
                printer.NewLine();
            }
        
    }
}
